...
actionable improvement suggestions. Be concise and direct.

Number each suggestion. Do not use markdown bold or special formatting.
Write in clean plain prose.

1. [Short title]: [suggestion]
2. [Short title]: [suggestion]
3. [Short title]: [suggestion]
...
# --- Main Analysis ---
def analyze(resume_paste, job_description):
    resume_text = resume_paste.strip() if resume_paste else ""

    if not resume_text:
        return (
            '<div style="border:1px solid #FF6B6B; border-radius:8px; padding:20px; background:#fff0f0; color:#FF6B6B;">Please paste your resume text.</div>',
    ...
    gr.HTML("""
        <div style="text-align:center; padding:32px 0 16px 0;">
            ...
            <p style="font-size:14px; color:#888; max-width:700px; margin:0 auto;">
                Paste your resume and job description to receive your ATS score,
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            keywords_output = gr.HTML(
                value='<div style="border:1px solid #e0e0e0; border-radius:8px; padding:20px; color:#aaa;">Keyword comparison will appear here.</div>'
            )
            suggestions_output = gr.HTML(
                value='<div style="border:1px solid #e0e0e0; border-radius:8px; padding:20px; color:#aaa;">Improvement suggestions will appear here.</div>'
            )

    analyze_btn.click(
        fn=analyze,
        inputs=[resume_text, job_input],
